...
void k_sum(const long long k, ScapegoatTree::Node *node, long long accumulated_left_sum)
{
    ...
    long long node_value = node->data;
    long long sum_to_left = accumulated_left_sum;

    sum_to_left += (node->left ? node->left->subtreeDataSum : 0);

    long long sum_to_right = tree->root->subtreeDataSum - sum_to_left;
    long long k_left_sum = sum_to_left * k;

    if (k_left_sum >= sum_to_right)
    {
        if (node->left)
        {
            closestSum = std::min(closestSum, k_left_sum - sum_to_right);
            k_sum(k, node->left, accumulated_left_sum);
        }
    }
    else
    {
        sum_to_right -= node_value;
        k_left_sum += node_value * k;
        if (k_left_sum < sum_to_right)
        {
            accumulated_left_sum += (node->left ? node->left->subtreeDataSum : 0) + node_value;
            k_sum(k, node->right, accumulated_left_sum);
        }
        else
        {
            closestSum = k_left_sum - sum_to_right;
            return;
        }
    }
}
