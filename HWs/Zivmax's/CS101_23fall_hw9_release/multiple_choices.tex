...
\begin{parts}
    ...
    \begin{choices}
        \CorrectChoice The implementation of topological sort requires $O(\abs{V})$ extra space.
        \CorrectChoice Any sub-graph of a DAG has a topological sorting.
        \CorrectChoice A DAG can have more than one topological sorting.
    ...
    \begin{choices}
        \CorrectChoice Create a graph from a rooted tree by assigning arbitrary directions to the edges.
        The graph is guaranteed to have a valid topological order.
        \CorrectChoice A critical path in a DAG is a path from the source to the sink with the minimum total weights.
        \choice A DAG with all different weighted edges has one unique critical path.
        \CorrectChoice Let $c(G)$ be the run time of finding a critical path and $t(G)$ be the run time of finding an arbitrary topological sort in a DAG $G$, then $c(G)=\Theta(t(G))$.
    \end{choices}
    ...
    \begin{choices}
        ...
        \CorrectChoice If $\forall i\in[3,n], c_i=c_{i-1}+c_{i-2}$, then $\forall n,g^*(n)=g(n)$.
