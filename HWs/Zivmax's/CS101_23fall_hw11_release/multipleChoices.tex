...
\begin{choices}
    \CorrectChoice The Floyd-Warshall algorithm has a time complexity of $\Theta(|V|^3)$.
    ...
    \CorrectChoice We can modify Floyd-Warshall algorithm to detect whether two vertices are strongly connected in a directed graph.
\end{choices}
...
\begin{choices}
    \CorrectChoice The greedy algorithm that solves the interval scheduling problem fails on the weighted interval scheduling problem.
    \CorrectChoice If there are $n$ houses and $m$ colors, the house coloring problem can be solved in $\Theta(nm)$ time complexity.
...
\begin{choices}
    \CorrectChoice If we use brute-force algorithm to compute $OPT(n)$, then the worst-case time complexity is $\Theta\left(\left(\dfrac{1+\sqrt{5}}{2}\right)^n\right)$.
    ...
    \CorrectChoice No matter whether we use top-down dynamic programming (memorization) or bottom-up dynamic programming to compute $OPT(n)$, the time complexity is $\Theta(n)$, excluding the time for sorting and computing $p(j)$.
